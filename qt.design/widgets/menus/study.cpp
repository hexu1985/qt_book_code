...
Study::Study(QWidget* parent) : QMainWindow(parent) {
    ...
    QWidget::setWindowTitle( "to become a jedi, you wish?  " );                         /* The some of the ClassName:: prefixes used here are
not necessary, because the functions can be called on "this".
The class names can be used to explicitly call a baser-version,
or show the reader which version is called. */
    ...
    useMenu->addAction(useTheForce);    /* It's already in a
      QActionGroup, but we also add it to a QMenu. */
//end
...
//start id=study
    ...
    toolbar = new QToolBar("Choice ToolBar", this); /* This gives
        us visible buttons in a dockable widget for
        each of the QActions. */
    ...
    QObject::connect(actionGroup, SIGNAL(triggered(QAction*)),
            ...
            connecting each individual action's signal, 
            perform one connect to an actionGroup that
            contains them all. */
