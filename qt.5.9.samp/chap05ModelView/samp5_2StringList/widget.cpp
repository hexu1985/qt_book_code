...
//    QModelIndex  index;
//    QString     str;
//    ui->plainTextEdit->clear(); //清除 plainTextEdit 的文本

//    for (int i=0; i<theModel->rowCount();i++) //逐行处理
//    {
//        index=theModel->index(i,0);//构造 ModelIndex
//        str=theModel->data(index,Qt::DisplayRole).toString();  //通过ModelIndex获取一行数据
//        ui->plainTextEdit->appendPlainText(str); //添加到plainTextEdit
//    }
}
...
//void Widget::on_btnListRole_clicked()
//{//设置角色，但是不起作用
//    QModelIndex  index;
...
////        theModel->setData(index,Qt::red,Qt::DecorationRole);
//        theModel->setData(index,"Help",Qt::ToolTipRole);//不起作用
//        theModel->setData(index,Qt::AlignRight,Qt::TextAlignmentRole);
//        theModel->setData(index,Qt::Checked,Qt::CheckStateRole);
//    }
